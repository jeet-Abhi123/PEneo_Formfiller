...
def fetcher_RobertaTokenizer(orig_text: str, tokens: List[str]) -> List[str]:
    ...
    for i, token in enumerate(tokens):
        ...
        if token == "<unk>":
            while orig_index < len(orig_text) and orig_text[orig_index] == " ":
                real_token += orig_text[orig_index]
                orig_index += 1
            if orig_index < len(orig_text):
                real_token += orig_text[orig_index]
            ...
            if token.startswith("Ġ"):
                # Only add space if not the first token (i > 0)
                # This prevents adding a space at the beginning of text
                token = token.replace("Ġ", " ") if i > 0 else token.replace("Ġ", "")
            for c in token:
                while orig_index < len(orig_text) and c != orig_text[orig_index] and c.upper() != orig_text[orig_index]:
                    real_token += orig_text[orig_index]
                    orig_index += 1

                if orig_index >= len(orig_text):
                    # If we've reached the end of original text, append remaining token chars
                    real_token += c
                    continue
